...
- Admin UI shows a deterministic read-only `secure_key` and a configuration form to select which tables to include and set the batch size.

---

## Requirements

- PrestaShop 1.7+ (module tested against PrestaShop 1.7 / 8 / 9 environments).
...
- `Module secure key` (read-only): deterministic key calculated from your shop configuration. Use this in cron URLs.
...
- If you receive `Invalid token` in the JSON response, verify that `secure_key` matches the CLI computed value shown above.
...
See `CHANGELOG.md` in the module folder for release notes and details about recent changes.
